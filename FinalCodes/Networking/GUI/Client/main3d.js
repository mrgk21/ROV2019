/**
 * Math helper for converting degrees to radians
...
/**
 * Math helper for converting radians to degrees
...
/**
 * Math helper function for rotating a point around another point by an angle mentioned in degrees.
 * @param {Number} x reference Point
 * @param {Number} y point to rotate
 * @param {Number} angle angle in degrees by which it is to be rotated
 */
function rotatePoint(x, y, angle) {
	return {
		x: x * Math.cos(d2r(angle)) - y * Math.sin(d2r(angle)),
		y: x * Math.sin(d2r(angle)) + y * Math.cos(d2r(angle)),
	};
}

/**
 * Math angle clamping utility function
 * @param {Number} value value to be clamped
 * @param {Number} min minimum allowable value
 * @param {Number} max maximum allowable value
 * @param {String} mode mode of restriction(circular/zero)
 */
...
//anonymus function to be called on load
$(function () {
	// CIRULAR COMPASSES
	...
	/**
	 * Draw the background circles for all the three parameters in GUI.
	 * @param {Context} ctx canvas context
	 * @param {String} color1 outline color
	 * @param {String} color2 fill color
	...
	/**
	 *	A pentagonal marker for the rings drawn on canvas
	 * @param {Context} ctx canvas context
	 * @param {Number} x reference point for pentagonal marker
	 * @param {Number} y reference point for pentagonal marker
	 * @param {Number} direction enum to define directions
	 */
	...
	/**
	 * Addign text to the canvases
	 * @param {Context} ctx canvas context
	 * @param {String} text text to be put as the proprty name
	 * @param {String} value actual value of that property
	 * @param {Number} x reference point x coordinate
	 * @param {*} y reference point y coordinate
	 * @param {*} offset offset between the text and value
	 */
	...
	//Style references: "#325c80" "#4682b4"
	//YAW CANVAS
	function ctx1update() {
		...
		*/
		...
		/**
		 * PentagonMarker Update Logic
		 * The whole pentagon marker needs to be rotated about a point(reference point) by a certain amount.
		 * As the pentagon is drawn using the path, each point's new position is determined first and then they are connected.
		 */
		/***************** DO NOT MESS WITH THIS EVER ********************/
		var radius = 60;
		var points = [
			{
				x: 0,
				y: 5,
				radius: Math.sqrt(radius * radius + 25),
				angel: r2d(Math.atan2(5, radius)),
			},
			{
				x: 0,
				y: -5,
			...
			},
			{
				x: 10,
				y: -5,
				radius: Math.sqrt((radius + 10) * (radius + 10) + 25),
				angel: -r2d(Math.atan2(5, radius + 10)),
			},
			{ x: 15, y: 0, radius: radius + 15, angel: 0 },
			{
				x: 10,
				y: 5,
				radius: Math.sqrt((radius + 10) * (radius + 10) + 25),
		...
		points.forEach((element) => {
			newX1 =
				element.radius *
					Math.cos(d2r(element.angel)) *
					Math.cos(-d2r(yaw - 90)) -
				element.radius *
					Math.sin(d2r(element.angel)) *
					Math.sin(-d2r(yaw - 90));
			newY1 =
				element.radius *
					Math.cos(d2r(element.angel)) *
	...
	function ctx3update() {
		...
		points.forEach((element) => {
			...
			newX2 =
				element.radius *
					Math.cos(d2r(element.angel)) *
					Math.cos(d2r(-roll + 180)) -
				element.radius *
					Math.sin(d2r(element.angel)) *
					Math.sin(d2r(-roll + 180));
			newY2 =
				element.radius *
					Math.cos(d2r(element.angel)) *
					Math.sin(d2r(-roll + 180)) +
	...
	//DO NOT CHANGE THIS otherwise camera configureation is not applied.
